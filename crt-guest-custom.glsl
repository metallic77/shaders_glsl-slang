...
float sw(float y, float lum) 
{
	float dy = y; 							// fract(OGL2Pos)
	float scan = scanline;			
	float beam = mix(beam_min,beam_max,lum);		

	dy = exp2(-scan * pow(dy,beam));
	return dy;
}
...
void main()
{
	vec2 ps = SourceSize.zw;						//InputSize.xy
	vec2 OGL2Pos = TEX0.xy * SourceSize.xy;			//vtexCoord.xy*TextureSize.xy
